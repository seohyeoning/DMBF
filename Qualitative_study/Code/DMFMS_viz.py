...
class Net(nn.Module):
    ...
    def forward(self, x, label=None, infer=False):
        ...
        last_feat = self.MSclassifier(fused_feat)
        MS_logit = F.softmax(last_feat, dim=1)

        if infer:
            return feat_list, weight_feature_list, weight_feature, fused_feat_3d, last_feat
    ...
    def infer(self, data_list):
        feat_list, weight_feature_list, weight_feature, fused_feat_3d, last_feat = self.forward(data_list, infer=True)
        return feat_list, weight_feature_list, weight_feature, fused_feat_3d, last_feat
