...
def Experiment(args):
    ...
    if args.CRL_user == True:
        res_dir = os.path.join (WD,'res',  f'{args.backbone}/{args.data_type}_{args.mode}/{args.model_type}') 
    
    else: # args.CRL_user == False
        res_dir = os.path.join (WD,'res',  f'{args.backbone}/woCRL/{args.data_type}_{args.mode}/{args.model_type}') 
...
if __name__ == "__main__":   
    ...
    parser.add_argument('--model_type', default='Model_OURS', choices=['Model_OURS']) # HJU_FE_OUR
    ...
    ###################################### FIX 
    parser.add_argument('--backbone', default='Transformer', choices = ['EEGNet4', 'DeepConvNet', 'Transformer', 'ResNet18'])
    ...
    parser.add_argument('--in_dim', default=[64], choices=[[64], [64,1,1,1]])
    parser.add_argument('--n_classes', default=2)
    parser.add_argument('--freq_time', default=750, help='frequency(250)*time window(3)') 
    parser.add_argument('--include_sbj', default=[1,4,5,8,13,14,15,16,18,19,20], help='drowsy 0인 sbj는 삭제') # drowsy 0인 sbj는 삭제
    parser.add_argument('--num_fold', default=4, help='number of folds') # original: 3, _7: 4
    ...
    if args.backbone == 'EEGNet4':
        if args.model_type == 'Model_OURS':
            if args.mode == 'uni':
                from Journal.DMFMS.Models.Model_OURS_EGN_SA import Net
            elif args.mode == 'multi':
                from Journal.DMFMS.Models.Model_OURS_EGN_LG import Net
