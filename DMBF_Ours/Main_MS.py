...
def Inference(args, MODEL_PATH_len):

    ##### SAVE DIR for .pt file
    if args.CRL_user == True:
        res_dir = os.path.join (WD,'res',  f'{args.backbone}/{args.data_type}_{args.mode}_{args.modal}/{args.model_type}') 
    
    else: # args.CRL_user == False
        res_dir = os.path.join (WD,'res',  f'{args.backbone}/woCRL/{args.data_type}_{args.mode}_{args.modal}/{args.model_type}') 
    ...
    num_fold = 4
    include_sbj = [5,6,9,10,13,15,16,18,20] # JBHI setting. # [5,6,8,9,10,13,15,16,17,18,19,20,21] : EMBC setting.
    for subj in range(1,24): # 2cl_5: (1,24)
        ...
        for nf in range(1, num_fold+1):
            ...
            print('='*30)
            print(' '*4, '피험자{} - fold{}을 test로 추론 시작'.format(subj, nf))
            print('='*30)
            ...
            MODEL_PATH = os.path.join(MODEL_PATH_len, res_name, nfoldname, FILENAME_MODEL)
            ...
            # Save Results
            trainer.save_result(tr_history=None, ts_history=ts_history, res_dir=res_dir)
...
def Experiment(args):
    ...
    ts_fold = pd.DataFrame(columns=METRICS)
    num_fold = 4
    include_sbj = [5,6,8,9,10,13,15,16,17,18,19,20,21] # 2cl_5 전용
    for subj in range(1,24): # 2cl_5: (1,24)
        if subj not in include_sbj:
                continue
        for nf in range(1, num_fold+1):
            ...
            # Save Results
            trainer.save_result(tr_history, ts_history=tr_history, res_dir=res_dir)
...
if __name__ == "__main__":   
    ...
    parser.add_argument('--modal', default = 'ALL', choices=['ALL', 'EEG', 'ECG', 'PPG', 'GSR', 'RSP'])
    ...
    if args.backbone == 'EEGNet4':
        ...
        if args.model_type == 'Model_OURS':
            from Models.Model_OURS_TF_LG import Net
    ...
    Experiment(args)
    # Inference(args, '/opt/workspace/Seohyeon/Journal/DMFMS/res/MS/DMBF/Model_OURS_CELoss_msp_CosineAnnealingLR_AdamW_0.002/0')
